	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			if (currentChar >= 'A' && currentChar <= 'Z') {
				...
				return String.format("*** Error *** An invalid character '%s' has been found!", currentChar);
			}
			if (currentCharNdx >= 7) {
		...
		String errMessage = "";
		if (!foundUpperCase)
			errMessage += "You need at least one uppercase letter;\n";
		
		if (!foundLowerCase)
			errMessage += "You need at least one lowercase letter;\n";
		
		if (!foundNumericDigit)
			errMessage += "You need a numeric digit;\n";
			
		if (!foundSpecialChar)
			errMessage += "You need a special character;\n";
			
		if (!foundLongEnough)
			errMessage += "Password has to be at least 8 characters;\n";
